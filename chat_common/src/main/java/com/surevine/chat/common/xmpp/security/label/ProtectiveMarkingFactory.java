 ...
 */
public class ProtectiveMarkingFactory {
    ...
    /**
     * Creates a new {@link ProtectiveMarking} from the given XML element.
     *
     * @param xmlElement
     *            The XML element.
     * @return The security label.
     */
    public ProtectiveMarking createProtectiveMarking(final IXmlElement xmlElement) {
        IXmlElement nameElement = xmlElement.getFirstChild(NAME);
        if (nameElement == null) {
            return null;
        }
        return new ProtectiveMarking(nameElement.getText());
    }
